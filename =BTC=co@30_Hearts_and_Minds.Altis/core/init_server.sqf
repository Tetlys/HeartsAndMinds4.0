...
if (btc_p_db_autoRestart > 0) then {
    [{
        ...
        [btc_db_fnc_autoRestart, [], 10 * 60] call CBA_fnc_waitAndExecute;
    }, [], btc_p_db_autoRestartTime * 60 * 60 - 10 * 60] call CBA_fnc_waitAndExecute;
};
...
if isServer then
{
	[] spawn
	{
		while {true} do{0 setFog 0;sleep 60;};
  };
};
